    ...
    , IPhongThuyNhanSinhService phongThuyNhanSinhService) : BaseHttpBusiness<BocMenhBusiness, ApplicationDbContext>(logger, contextFactory, contextAccessor), IBocMenhBusiness
{
    ...
    private async Task<(string systemPrompt, string userPrompt, string combinedPrompts, LaSoBatTuResponse laSoBatTu)> GenerateBatTuTuTruPromptLegacyAsync(TuTruBatTuRequest request)
    {
        ...
        TuTruInfo tutru = laSoBatTu.Tutru;
...
11. **Luôn chú trọng JSON syntax để không tạo ra lỗi. sau khi có kết quả trả về phải kiểm tra lại một lần nữa liệu có lỗi JSON syntax nào có thể khiến quá trình JsonSerializer.Deserialize gây lỗi trước khi trả về, nếu có hãy fix nó.**

== CẤU TRÚC JSON BẮT BUỘC ==
Hãy điền thông tin vào mẫu JSON dưới đây. **sau khi có kết quả phải kiểm tra lại một lần nữa liệu có lỗi JSON syntax nào có thể khiến quá trình JsonSerializer.Deserialize gây lỗi**
